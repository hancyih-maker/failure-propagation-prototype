      ...
      return `
        <article class="module-card ${item.id === selectedModuleId ? "is-selected" : ""}" data-module-id="${item.id}">
          <button class="module-main-button" type="button" data-module-select="${item.id}">
            ...
            ${sourceAnnotations.length ? `<span class="source-taxonomy-list">${sourceAnnotations.map((item) => `<span class="taxonomy-chain source-taxonomy-item" data-source-point="${escapeHtml(item.pointKey)}">${badge(item.firstLevel)}<strong>${escapeHtml(item.secondLevel)}</strong>${escapeHtml(item.scope || "Scope not set")}</span>`).join("")}</span>` : ""}
          </button>
          ...
          ${downstreamTaxonomy.length ? `<span class="downstream-label-strip">${downstreamTaxonomy.map((item) => `<span class="downstream-taxonomy-item taxonomy-chain" data-target-point="${escapeHtml(item.pointKey)}">${badge(item.firstLevel)}<strong>${escapeHtml(item.secondLevel)}</strong>${escapeHtml(item.scope || "Scope not set")}</span>`).join("")}</span>` : ""}
        </article>
      `;
    }).join("");
    return `
    ...
    function renderStages() {
      ...
      stageLayer.innerHTML = data.stages.map(function(stage) {
        var cards = data.modules.filter(function(item) { return item.stage === stage.id; }).map(function(item) {
          ...
          var sourceHtml = sourceAnnotations.length ? "<span class=\"source-taxonomy-list\">" + sourceAnnotations.map(function(annotation) {
            return "<span class=\"taxonomy-chain source-taxonomy-item\" data-source-point=\"" + esc(annotation.pointKey) + "\">" + badge(annotation.firstLevel) + "<strong>" + esc(annotation.secondLevel) + "</strong>" + esc(annotation.scope || "Scope not set") + "</span>";
          }).join("") + "</span>" : "";
          ...
          var downstreamHtml = downstreamTaxonomy.length ? "<span class=\"downstream-label-strip\">" + downstreamTaxonomy.map(function(annotation) {
            return "<span class=\"downstream-taxonomy-item taxonomy-chain\" data-target-point=\"" + esc(annotation.pointKey) + "\">" + badge(annotation.firstLevel) + "<strong>" + esc(annotation.secondLevel) + "</strong>" + esc(annotation.scope || "Scope not set") + "</span>";
          }).join("") + "</span>" : "";
